...
$sel = mysqli_query($con,"select count(*) as allcount from examination_special_cases");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of record with filtering
$sel = mysqli_query($con,"select count(*) as allcount from examination_special_cases WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from examination_special_cases WHERE 1 ".$searchQuery." order by id DESC
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
        ...
        "email_address"=>$row['email_address'].'<br/>'.$row['telephone'].'<br/>
        '.$row['profession'].'<br/> Year of Completion:'.$row['completion_year'],
        "reason"=>$row['reason'].', '.$row['other_reason'],
