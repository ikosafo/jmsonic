...
## Total number of records without filtering
            $sel = mysqli_query($con,"select count(*) as allcount from examination_reg e JOIN provisional p ON e.applicant_id = p.applicant_id
                   where exam_type = 'Supplementary' AND e.period_registered BETWEEN '$startdate' AND '$enddate'");
            $records = mysqli_fetch_assoc($sel);
            $totalRecords = $records['allcount'];
...
## Fetch records
            ...
            exam_type = 'Supplementary' AND e.period_registered BETWEEN '$startdate' AND '$enddate' AND 1 ".$searchQuery."
            order by e.period_registered DESC,e.payment DESC limit ".$row.",".$rowperpage;
            $empRecords = mysqli_query($con, $empQuery);
            $data = array();
...
## Fetch records
            $empQuery = "select * from examination_reg e JOIN provisional p ON e.applicant_id = p.applicant_id
            WHERE exam_type = 'Supplementary' AND e.period_registered BETWEEN '$startdate' AND '$enddate' AND  payment = '1'
            AND (exam_usercheck_status = '' OR exam_usercheck_status IS NULL)
            AND 1 ".$searchQuery." order by e.period_registered DESC,e.payment DESC limit ".$row.",".$rowperpage;
            $empRecords = mysqli_query($con, $empQuery);
            $data = array();
