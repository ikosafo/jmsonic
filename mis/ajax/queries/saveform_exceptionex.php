...
$other_reason = mysqli_real_escape_string($mysqli, $_POST['other_reason']);
...
$getid = $mysqli->query("select * from examination_special_cases ORDER BY id DESC LIMIT 1");
$resid = $getid->fetch_assoc();
$lastid = $resid['id'];

$randomid = $lastid + 1;
$random = "Ex" . rand(1, 10) . $randomid . date('mdy');

$datetime = date("Y-m-d H:i:s");

$count = $mysqli->query("select * from examination_special_cases where email_address = '$email_address'");
$res_count = mysqli_num_rows($count);

if ($res_count == "0") {

    $mysqli->query("INSERT INTO `examination_special_cases`
            (`surname`,
             `firstname`,
             `othername`,
             `email_address`,
