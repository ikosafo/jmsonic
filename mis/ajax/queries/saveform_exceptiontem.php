...
$getid = $mysqli->query("select * from temporal_special_cases ORDER BY id DESC LIMIT 1");
$resid = $getid->fetch_assoc();
$lastid = $resid['id'];
$randomid = $lastid + 1;
$random = "Tm" . rand(1, 1000) . $randomid . date('mdy');

$datetime = date("Y-m-d H:i:s");

$count = $mysqli->query("select * from temporal_special_cases where email_address = '$email_address'");
$res_count = mysqli_num_rows($count);


if ($res_count == "0") {
    $mysqli->query("INSERT INTO `temporal_special_cases`
            (`surname`,
             `firstname`,
             `othername`,
             `email_address`,
...
VALUES ('$surname',
    ...
    Professions Council.</b>. The code to use for your temporal registration is <b>$random</b>. <br/>
    Log in to the temporal registration portal with the code. </p>
    <p>Thank you.</p>
";
