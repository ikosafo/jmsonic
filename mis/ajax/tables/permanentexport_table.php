...
if ($year_search == 'All') {
    $query = $mysqli->query("Select * from provisional where provisional_pin is not null and permanent_registration = '1'");
}

else {
    $query = $mysqli->query("Select * from provisional where SUBSTRING(permanent_period,1,4) = '$year_search'
...
<script>
    ...
    var KTDatatablesExtensionButtons = {
        init: function () {
            ...
            if (year_search == 'All') {
                $('#prov_table').append('<caption style="caption-side: top">Permanent Registration Pin Export for all years</caption>');
            }
            else {
                $('#prov_table').append('<caption style="caption-side: top">Permanent Registration Pin Export for the year <?php echo $year_search ?></caption>');
            }
