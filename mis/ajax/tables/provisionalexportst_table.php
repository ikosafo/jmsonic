...
if ($search_status == 'Any') {
    $query = $mysqli->query("Select * from provisional where provisional_registration = '1' ORDER BY provisional_date_approval DESC");
}
else if ($search_status == 'Approved') {
    $query = $mysqli->query("Select * from provisional where provisional_registration = '1'
                          and provisional_admincheck_status = 'Approved' ORDER BY provisional_date_approval DESC");
}
else if ($search_status == 'Rejected') {
    $query = $mysqli->query("Select * from provisional where provisional_registration = '1'
                          and (provisional_admincheck_status = 'Rejected' or provisional_usercheck_status = 'Rejected')
                          ORDER BY provisional_date_approval DESC");
}
else {
    $query = $mysqli->query("Select * from provisional where provisional_registration = '1'
        ...
        <div class="table-responsive">
            <table class="table" id="prov_table">
                ...
                <tr>
                    ...
                    <th>Mis Comment/Reason</th>
                    <th>Super Admin<br/> Approved</th>
                    <th>Admin Comment/Reason</th>
...
<script>
    ...
    var KTDatatablesExtensionButtons = {
        init: function () {
            ...
            $('#prov_table').append('<caption style="caption-side: top">Provisional Registration Export (<?php echo $search_status ?>)</caption>');
