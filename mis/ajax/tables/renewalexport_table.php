...
if ($renewal_status == "Approved") {
    $query = $mysqli->query("SELECT * FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.cpd_admincheck_status = 'Approved'");
}
else if ($renewal_status == "Rejected") {
    $query = $mysqli->query("SELECT * FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.cpd_admincheck_status = 'Rejected'");
} else if ($renewal_status == "Not Paid") {
    $query = $mysqli->query("SELECT * FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.payment = '0'");
} else {
    $query = $mysqli->query("SELECT * FROM renewal r
                              JOIN provisional p ON r.applicant_id = p.applicant_id
                              WHERE r.cpdyear != '' AND r.cpdyear = '$year_search' AND r.payment = '1'
                              AND (r.cpd_admincheck_status != 'Rejected' OR r.cpd_admincheck_status != 'Approved')");
}
...
<script>
    ...
    var KTDatatablesExtensionButtons = {
        init: function () {
            ...
            if (year_search == 'All') {
                $('#prov_table').append('<caption style="caption-side: top">Provisional Registration Pin Export for all years</caption>');
            }
            else {
                $('#prov_table').append('<caption style="caption-side: top">Provisional Registration Pin Export for the year <?php echo $year_search ?></caption>');
            }
